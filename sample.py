...
def dashboard_transaction():
  ...
  with st.container():
    ...
    with col1:
      radio_button = st.radio('Select graph type', ['By Amount', 'By Number of Transactions'])
      
    with col2:
      

      if radio_button == 'By Amount':
        fig_year_amount = px.bar(AT_grouped, x="Year", y=["Amount"],
                                title="Transaction Amount by Year from 2018 to 2023",
                                range_y=[0,max(AT_grouped['Amount']) * 1.1])
      elif radio_button == 'By Number of Transactions':
        fig_year_amount = px.bar(AT_grouped, x="Year", y=["Count"],
                                title="Transaction Count by Year from 2018 to 2023",
                                range_y=[0,max(AT_grouped['Count']) * 1.1])
        
      st.plotly_chart(fig_year_amount)

def total_ai():
    sql_query = "SELECT SUM(Amount) AS Total_Amount FROM aggregated_insurance"
    cursor.execute(sql_query)
    total_amount_row = cursor.fetchone()
    total_amount_ai = total_amount_row[0]
    total_amount_float = float(total_amount_ai)
    total_amount_crores = np.round(total_amount_float / 10_000_000,2)
    st.header('Total Transaction: ' + str(total_amount_crores) + ' Crores') 
